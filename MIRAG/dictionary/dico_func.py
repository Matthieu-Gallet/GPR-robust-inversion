...
def ricky(er, R, tmax, xmax, pxmax, pymax, f):
    """

    Parameters
    ----------
    er : permittivity of the medium
    R : radius of the sphere
    tmax : range time of the vertical acquisition (s)
    xmax : range of the horizontal acquisition (m)
    pxmax : number of samples in the horizontal acquisition
    pymax : number of samples in the vertical acquisition
    f : frequency of the acquisition (Hz)

    Returns
    -------
    r: meshgrid of the atoms
        _description_
    """
    x, t = np.meshgrid(np.linspace(0, xmax, pxmax), np.linspace(0, tmax, pymax))
    t_0 = tmax / 4
    x_0 = xmax / 2
    om = f * 2 * np.pi
    cr = 0 * 1.45e6  # conductivity of the medium
    v = 3e8 / (np.sqrt((er / 2) * (np.sqrt(1 + (cr / om) ** 2) + 1)))  # *1e-9)
    a = 2 / v
    p = (t_0 * v / 2) + R
    # Defining g using equation (4)
    g = a * np.sqrt(p**2 + (x - x_0) ** 2) + t_0 - p * a
    # Defining ricker wavelet from equation (3)

    r = (1 - (0.5 * (om**2) * ((t - g) ** 2))) * np.exp(
        (-0.25) * (om**2) * ((t - g) ** 2)
    )
    return r
